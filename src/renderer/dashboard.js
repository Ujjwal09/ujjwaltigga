...
function initCoach() {
  const host = el('coach');
  host.innerHTML = '<div class="hint">Click “Generate summary” for a motivational read on your learning curve and what to improve — generated from your tracked data.</div>';
  el('coachBtn').onclick = async () => {
    const btn = el('coachBtn');
    btn.disabled = true;
    host.innerHTML = '<div class="loading"><span class="spin"></span> Reading your data and thinking…</div>';
    const res = await window.tg.coach();
    btn.disabled = false;

    if (!res.ok) {
      if (res.code === 'no_key') {
        host.innerHTML = '<div class="hint">No OpenAI key found. Either set the <b>OPENAI_API_KEY</b> environment variable, or create <b>~/.timegrid/config.json</b> containing <code>{ "OPENAI_API_KEY": "sk-..." }</code>, then click Generate again. Your key stays on your machine.</div>';
      } else if (res.code === 'no_data') {
        host.innerHTML = '<div class="hint">No tracked time yet — tick some slots in the tracker, then come back for your first summary.</div>';
      } else {
        host.innerHTML = `<div class="hint">Couldn't generate a summary: ${escapeHtml(res.message || 'unknown error')}. Check your API key and connection, then try again.</div>`;
      }
      return;
    }

    const d = res.data;
    host.innerHTML = `<div class="coach">
      <div class="headline">${escapeHtml(d.headline)}</div>
      <div class="stats">${escapeHtml(d.stats)}</div>
      <h3>Your learning curve</h3>
      <p>${escapeHtml(d.learning)}</p>
      <h3>Where to improve</h3>
      <p>${escapeHtml(d.improve)}</p>
      <h3>Try this</h3>
      <ul class="actions">${(d.actions || []).map(a => `<li>${escapeHtml(a)}</li>`).join('')}</ul>
      <div class="bottom">${escapeHtml(d.bottom)}</div>
      <div class="cmeta"><span>Generated by OpenAI from your tracked data</span></div>
    </div>`;
  };
}
